...
#define STANDALONE_MT76_NPU_RX_BUF_SIZE	(1800 + \
...
int standalone_mt76_npu_fill_rx_queue(struct standalone_mt76_dev *dev, struct standalone_mt76_queue *q)
{
	...
	while (q->queued < q->ndesc - 1) {
		...
		struct standalone_mt76_queue_entry *e = &q->entry[q->head];
		struct page *page;
		int offset;

		e->buf = standalone_mt76_get_page_pool_buf(q, &offset, q->buf_size);
		if (!e->buf)
			break;
...
void standalone_mt76_npu_queue_cleanup(struct standalone_mt76_dev *dev, struct standalone_mt76_queue *q)
{
	spin_lock_bh(&q->lock);
	while (q->queued > 0) {
		struct standalone_mt76_queue_entry *e = &q->entry[q->tail];

		dma_sync_single_for_cpu(dev->dma_dev, e->dma_addr[0],
					e->dma_len[0],
					page_pool_get_dma_dir(q->page_pool));
		standalone_mt76_put_page_pool_buf(e->buf, false);
...
static struct sk_buff *standalone_mt76_npu_dequeue(struct standalone_mt76_dev *dev,
					...
					u32 *info)
{
	...
	for (i = 0; i < nframes; i++) {
		struct standalone_mt76_queue_entry *e = &q->entry[index];
...
EXPORT_SYMBOL_GPL(standalone_mt76_npu_check_ppe);

static int standalone_mt76_npu_rx_poll(struct napi_struct *napi, int budget)
{
	struct standalone_mt76_dev *dev = standalone_mt76_priv(napi->dev);
	enum standalone_mt76_rxq_id qid = napi - dev->napi;
	...
	while (done < budget) {
		...
		skb = standalone_mt76_npu_dequeue(dev, &dev->q_rx[qid], &info);
		if (!skb)
			break;
	...
	standalone_mt76_npu_fill_rx_queue(dev, &dev->q_rx[qid]);
out:
...
static irqreturn_t standalone_mt76_npu_irq_handler(int irq, void *q_instance)
{
	struct standalone_mt76_queue *q = q_instance;
	struct standalone_mt76_dev *dev = q->dev;
...
void standalone_mt76_npu_queue_setup(struct standalone_mt76_dev *dev, struct standalone_mt76_queue *q)
{
	...
	bool xmit = standalone_mt76_queue_is_npu_tx(q);
	struct airoha_npu *npu;
...
int standalone_mt76_npu_rx_queue_init(struct standalone_mt76_dev *dev, struct standalone_mt76_queue *q)
{
	...
	err = dev->queue_ops->alloc(dev, q, 0, size,
				    STANDALONE_MT76_NPU_RX_BUF_SIZE, 0);
	if (err)
		goto out;

	name = devm_kasprintf(dev->dev, GFP_KERNEL, "standalone_mt76-npu.%d", index);
	if (!name) {
		err = -ENOMEM;
		goto out;
	}

	err = devm_request_irq(dev->dev, irq, standalone_mt76_npu_irq_handler,
			       IRQF_SHARED, name, q);
	if (err)
		goto out;

	netif_napi_add(dev->napi_dev, &dev->napi[qid], standalone_mt76_npu_rx_poll);
...
EXPORT_SYMBOL_GPL(standalone_mt76_npu_rx_queue_init);

static int standalone_mt76_npu_setup_tc_block_cb(enum tc_setup_type type,
				      void *type_data, void *cb_priv)
{
	struct standalone_mt76_phy *phy = cb_priv;

	if (type != TC_SETUP_CLSFLOWER)
		return -EOPNOTSUPP;

	if (!standalone_mt76_ppe_device_active(phy->dev))
		return -EOPNOTSUPP;
...
static int standalone_mt76_npu_setup_tc_block(struct standalone_mt76_phy *phy,
				   struct net_device *dev,
				   struct flow_block_offload *f)
{
	flow_setup_cb_t *cb = standalone_mt76_npu_setup_tc_block_cb;
			  ...
			  void *type_data)
{
	...
	if (!standalone_mt76_npu_device_active(phy->dev))
		return -EOPNOTSUPP;

	switch (type) {
	case TC_SETUP_BLOCK:
	case TC_SETUP_FT:
		return standalone_mt76_npu_setup_tc_block(phy, dev, type_data);
	default:
		return -EOPNOTSUPP;
	}
}
EXPORT_SYMBOL_GPL(standalone_mt76_npu_net_setup_tc);
...
EXPORT_SYMBOL_GPL(standalone_mt76_npu_send_txrx_addr);
...
EXPORT_SYMBOL_GPL(standalone_mt76_npu_disable_irqs);
...
int standalone_mt76_npu_init(struct standalone_mt76_dev *dev, phys_addr_t phy_addr, int type)
{
	...
	dev->hwrro_mode = is_standalone_mt7996(dev) ? STANDALONE_MT76_HWRRO_V3 : STANDALONE_MT76_HWRRO_V3_1;
...
EXPORT_SYMBOL_GPL(standalone_mt76_npu_init);
...
void standalone_mt76_npu_deinit(struct standalone_mt76_dev *dev)
{
	...
	standalone_mt76_npu_queue_cleanup(dev, &dev->q_rx[MT_RXQ_NPU0]);
	standalone_mt76_npu_queue_cleanup(dev, &dev->q_rx[MT_RXQ_NPU1]);
}